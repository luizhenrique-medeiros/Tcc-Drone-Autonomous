...
export function VehiclesPage() {
  ...
  const operationalAlerts = (data ?? []).flatMap(({ vehicle, health, healthError }) =>
    generateOperationalAlerts({
  ...
  return (
    <>
      ...
      {error && data ? <Feedback tone="error" className="page-feedback">{error} · os dados anteriores permanecem identificados abaixo.</Feedback> : null}
      ...
      {data ? (
        <div className="vehicles-stack">
          {data.map(({ vehicle, health, healthError }) => {
            ...
            >
              {!health ? (
                <StateView state="error" compact title="Leitura indisponível" description={`${healthError || 'A API não retornou uma leitura.'} A autorização de voo está bloqueada para este veículo.`} />
              ) : (
                <>
                  <div className={`vehicle-readiness ${readiness.ready ? 'vehicle-readiness--ok' : 'vehicle-readiness--blocked'}`}>
                    {readiness.ready ? <CheckCircle2 size={25} /> : <TriangleAlert size={25} />}
                    <div>
                      <strong>{readiness.ready ? 'Leitura atual atende aos mínimos técnicos' : 'Leitura atual possui bloqueios'}</strong>
                      <span>{readiness.ready ? 'O checklist humano e a validação do backend ainda são obrigatórios.' : readiness.blockers.join(' ')}</span>
                    </div>
                  </div>
                  <div className="diagnostics-grid">
                    ...
                    <Diagnostic icon={<ShieldCheck />} label="Geofence" value={health.geofence_enabled === true ? 'Habilitada' : health.geofence_enabled === false ? 'Desabilitada' : '--'} detail="Nunca desabilitar para contornar operação" ok={health.geofence_enabled === true} />
                    <Diagnostic icon={<RefreshCw />} label="RTL" value={health.rtl_configured === true ? 'Configurado' : health.rtl_configured === false ? 'Não configurado' : '--'} detail="Área de retorno ainda exige conferência humana" ok={health.rtl_configured === true} />
                    <Diagnostic icon={<Clock3 />} label="Última amostra" value={formatDateTime(health.measured_at)} detail={`Recebida ${formatDateTime(health.received_at)} · ${health.is_stale ? 'DADO VENCIDO' : 'fresca segundo a API'}`} ok={!health.is_stale && Boolean(health.received_at)} />
                  </div>
                  {health.preflight_messages.length > 0 ? (
                    ...
                    <Feedback tone="warning" className="page-feedback">Mensagens de pré-arm indisponíveis nesta amostra.</Feedback>
                  )}
                </>
              )}
            </Card>
